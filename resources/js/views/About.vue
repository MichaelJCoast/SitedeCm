...
<scrollUp />
  ...
  <div v-if="about[0]" class="container min-h-screen my-6 mx-auto px-8 md:px-14">
    ...
    <h1
      class="text-neutral-900 dark:text-white text-center capitalize font-semibold text-2xl sm:text-5xl mt-5">
        {{aboutList.title}}
    </h1>
    ...
    <div class="w-full px-2 sm:px-0 mt-4">
        <h1
      class="text-neutral-900 dark:text-white text-center capitalize font-semibold text-2xl sm:text-5xl mt-20">
        FAQ
    </h1>
    <div class="mx-auto w-full max-w-screen rounded-2xl mt-4">
      <Disclosure v-slot="{ open }">
        <DisclosureButton class="flex w-full justify-between border-b px-4 py-2 text-left text-base sm:text-lg font-medium text-neutral-900 dark:text-neutral-200 dark:hover:text-white focus:outline-none focus-visible:ring focus-visible:ring-red-500 focus-visible:ring-opacity-75"
            :class="open ? 'border-red-500' : 'border-neutral-700'"> 
          <span>Quem criou este website?</span>
          <ChevronUpIcon :class="open ? 'rotate-180 transform' : ''" class="h-5 w-5 text-neutral-900 dark:text-neutral-200 text-lg"
          />
        </DisclosureButton>
      ...
      <Disclosure v-for="question in questions" :key="question.id" as="div" class="mt-2" v-slot="{ open }">
        <DisclosureButton
          class="flex w-full justify-between border-b px-4 py-2 text-left text-base sm:text-lg font-medium text-neutral-900 dark:text-neutral-200 dark:hover:text-white focus:outline-none focus-visible:ring focus-visible:ring-red-500 focus-visible:ring-opacity-75"
                  :class="open ? 'border-red-500' : 'border-neutral-700'">
